...
>abstract="Use the Scoring Studio skill to create, configure, and publish custom lead scoring models through the Coworker chat interface."

The [_Scoring Studio_ skill](./skills.md#scoring-signals) in [!DNL Adobe Marketo Optimizer] provides an AI-native lead scoring solution that enables you to create, configure, and publish lead scoring models. The studio combines an agent-driven workflow with a visual UI — you can build scoring models through natural language prompts in the [Coworker chat interface](./chat-interface.md) or by interacting directly with the UI controls.
...
![Scoring Studio launched in Coworker chat interface](./assets/scoring-studio.png){width="700" zoomable="yes"}
